...
class NetworkUnitTesting(unittest.TestCase):
    ...
    def __run(self, network_name: str):
        ...
        for _, (address, username, password, pin_code) in specific_wallets_details().items():
            # Task
            otp_hash = tools.walletcreator.otp_hash(username, password, pin_code)
            otp_code = pyotp.TOTP(otp_hash).now()
            address, private_key, _ = tools.walletcreator.access(username, password, pin_code, otp_code)
